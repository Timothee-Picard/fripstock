...
export class ShopsService {
  ...
  async update(currentUser: CurrentUser, id: string, dto: UpdateShopDto) {
    ...
    await this.detailGerant(currentUser, id);
  ...
  async delete(currentUser: CurrentUser, id: string) {
    ...
    if (products > 0) {
      throw new ConflictException(
        `Cette boutique contient ${products} produit(s). Réassigne-les avant de la delete.`,
      );
    }
  ...
  private async detailGerant(currentUser: CurrentUser, id: string) {
    const shop = await this.prisma.shop.findFirst({
