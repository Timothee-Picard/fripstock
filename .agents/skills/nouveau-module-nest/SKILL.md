...
description: Scaffold un nouveau module NestJS (controller, service, module, DTOs) dans apps/api en respectant les conventions Fripstock — nommage anglais, scoping entreprise/boutique, guards de permissions. Use when adding a new domain module to the API.
---
...
Crée un nouveau module NestJS nommé `<nom-fourni-par-utilisateur>` dans `apps/api/src/`.

## Fichiers à créer

Dans `apps/api/src/<nom-fourni-par-utilisateur>/` :

- `<nom-fourni-par-utilisateur>.module.ts` — déclare le controller, le service, importe `PrismaModule`.
- `<nom-fourni-par-utilisateur>.controller.ts` — routes REST, avec les décorateurs de permission
  (`@RequirePermission('...')`) sur toute route d'écriture, en te basant sur la liste
  de permissions définie dans `CLAUDE.md`.
- `<nom-fourni-par-utilisateur>.service.ts` — logique métier, utilise `PrismaService` injecté.
...
1. **Scoping multi-tenant** : toute requête Prisma sur une ressource métier doit
   filtrer par `companyId` (et `shopId` quand pertinent), déduit du JWT via le
...
3. **Nommage** : modèles et champs Prisma en anglais (voir "Conventions de code" dans
