...
Ajoute un modèle `<nom-fourni-par-utilisateur>` à `apps/api/prisma/schema.prisma`.
...
- Nom du modèle en anglais, `PascalCase`.
- Champs en anglais, `camelCase`.
...
- Si le modèle appartient à une Entreprise ou une Boutique, ajoute la clé étrangère
  correspondante avec un index (`@@index([companyId])`), pour permettre le scoping
...
2. Génère la migration : `npx prisma migrate dev --name <nom-fourni-par-utilisateur>` (dans le
