...
```
Entreprise (le compte parent, un gérant)
 └─ Boutique(s) — points de vente physiques, stock affiché par boutique
     └─ Utilisateur — rattaché à UNE Entreprise, accès à N Boutiques avec permissions par boutique
```

- Une personne (User) appartient à **une seule** Entreprise.
- Un User peut avoir accès à **plusieurs** Boutiques de son Entreprise.
- Le **gérant** (`estGerant = true`) a tous les droits sur toutes les boutiques de son
  entreprise, sans passer par la table de permissions.
- Un **employé** a une ligne `AccesBoutique` par boutique à laquelle il a accès, avec un
...
- Un produit est créé **au niveau Entreprise** (pas directement rattaché à une boutique à
  la création). Le champ `boutiqueId` est **nullable**, et une action dédiée
...
- Deux modes de vente, champ `typeVente` : `ACHAT_REVENTE` ou `DEPOT_VENTE`.
  - `ACHAT_REVENTE` : la boutique a acheté l'article, champ `prixAchat` pertinent.
  - `DEPOT_VENTE` : l'article appartient à un client déposant via un `ContratDepot`,
    pas de `prixAchat`, une commission s'applique à la vente.
  - À valider au niveau service : `DEPOT_VENTE` exige un `contratDepotId` et refuse
    `prixAchat` ; `ACHAT_REVENTE` refuse `contratDepotId` et `deposantPaye`.
- Champs prix à bien distinguer (vu dans le fichier client existant) :
  - `prixVente` = prix affiché / prix de vente fixé (l'étiquette)
  - `prixVendu` = prix réellement encaissé (peut différer si négocié), rempli au moment
    du passage au statut "vendu"
- Un produit a une **quantité** (`quantite`, défaut 1). Souvent 1 (article unique de
...
- Champs `nom`, `description`, `commentaire` : texte libre.
...
- Trois flags booléens sur `Statut`, non désactivables par l'UI mais définis à la création
  ...
  l'utilisateur :
  - `estVente: boolean` — si vrai, passer un produit à ce statut **est** une vente : c'est
    à ce moment que `prixVendu` et `dateVente` sont acceptés/exigés, et c'est ce flag (et
  ...
  - `bloqueVente: boolean` — si vrai, impossible de faire passer ce produit à un statut
    `estVente = true` ou de modifier son prix vendu (utilisé pour "Rendu au client",
    "Retiré").
  - `sortStock: boolean` — si vrai, le produit ne compte plus dans l'inventaire actif /
...
- `Client` (déposant) : nom, prénom, contact, IBAN, `commissionDefaut` (%). Rattaché à
  l'Entreprise (pas à une boutique précise, un déposant peut avoir des articles dans
  plusieurs boutiques de l'entreprise).
- `ContratDepot` : lie un `Client` à une période (`dateDebut`, `dateFin`), une commission
  (copiée depuis `commissionDefaut` à la création mais modifiable pour ce contrat précis),
...
- **La commission est figée à la vente** : au passage à un statut `estVente = true`, copier
  la commission du contrat dans `Produit.commissionAppliquee`. Tous les relevés, exports et
...
- Champ `deposantPaye: boolean` sur `Produit` (pertinent seulement si `DEPOT_VENTE` et
...
- Notification automatique (job planifié) quand `dateFin` d'un contrat approche
  (`notifyBeforeDays` avant l'échéance).
- Quand un produit en dépôt est "rendu" : il sort de l'inventaire actif (`sortStock` du
  statut), passe en `bloqueVente = true`, ne doit plus jamais être vendable.

## Permissions (clés utilisées dans le JSON `AccesBoutique.permissions`)
...
Le gérant (`estGerant = true`) bypass entièrement cette table : toujours tous les droits,
sur toutes les boutiques de son entreprise.

**Produits non assignés (`boutiqueId = null`)** : ils n'appartiennent à aucune boutique,
donc aucune ligne `AccesBoutique` ne les couvre. Règle : un utilisateur qui possède la
...
- Modèles Prisma en français, `PascalCase` (ex: `Produit`, `ContratDepot`).
- Champs en français, `camelCase` (ex: `prixVente`, `dateVente`).
...
- Un module Nest par domaine (`produits/`, `boutiques/`, etc.), avec `*.controller.ts`,
  `*.service.ts`, `*.module.ts`, `dto/`.
- Toute requête sur une ressource métier doit être scopée à `entrepriseId` (et
  `boutiqueId` quand pertinent) déduit du JWT — jamais du body/params fournis par le
  client, pour éviter toute fuite de données entre entreprises.
- Plusieurs modèles n'ont **pas** de colonne `entrepriseId` : `ContratDepot` (via `Client`),
  `AttributOption` (via `AttributDefinition`), `CategorieAttribut`, `ValeurAttribut`,
  `ProduitAttributOption`, `HistoriqueStatut` (via `Produit`), `AccesBoutique` (via `User`).
  Pour ceux-là, le `where` doit **toujours** filtrer via la relation parente
  (`where: { client: { entrepriseId } }`) — un `where: { id }` seul sur ces tables est une
...
- Pas d'historique de mouvement produit entre boutiques (juste le champ `boutiqueId`
  courant).
