...
- **Commits en anglais au format Conventional Commits**, sans exception :
  `type(scope): subject` (scope facultatif), sujet impératif en minuscules,
  72 caractères maximum, sans point final. Le corps et les pieds sont aussi en anglais.
  Exemple : `chore: add codex project configuration`.
- **PR créées ou modifiées par Codex : titre et description en anglais.** Le titre
  suit aussi Conventional Commits pour permettre un squash conforme. Décrire le
  changement final et les vérifications effectuées.
- Avant un push ou une PR, vérifier les messages avec
  `make check-commits RANGE="<base>..HEAD"` sur toute la plage de la branche.
  `make check` ne vérifie pas les messages de commit. Corriger un message invalide
  avant publication ; ne pas ignorer le hook de commit.
